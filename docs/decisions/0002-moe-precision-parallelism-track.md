...
2. Raising the *realized* FP8 utilization of LLM matmuls — especially MoE grouped
   GEMMs — and generalizing the method across models.
...
whether FP8/FP4 actually delivers speedup, and precision is an unused knob for
balancing the synchronous-layer straggler.
...
- **Paper 1 (kernel):** utilization-aware FP8 — a heterogeneous-precision grouped
  GEMM that realizes speedup under small/skewed per-expert token counts.
...
- The differentiator vs. the field is industrial EP + FP8 experience plus the
  utilization/roofline framing; the track must claim a precise niche rather than
  the broad "mixed-precision experts" headline, and move fast.
...
- Industrial FP8 recipes carry IP: reproduce only on open models; commit no
  internal data, weights, or hostnames (see `docs/versioning/artifact-policy.md`).
